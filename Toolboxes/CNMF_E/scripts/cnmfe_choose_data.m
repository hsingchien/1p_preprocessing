...
elseif strcmpi(file_type, '.avi')
    ...
    Y = squeeze(Y);
    toc;
    fprintf('Time cost in convert data to *.mat file:   %.2f seconds\n', toc);
    fprintf('saving the *.mat file...\n')
    tic;
    Ysiz = size(Y)
    [tmp_dir, tmp_file, ~] = fileparts(nam);
    nam_mat = sprintf('%s%s%s.mat', tmp_dir, filesep, tmp_file);
    save(nam_mat,'Ysiz','Y','-v7.3');
    fprintf('Time cost in saving the data to *.mat file: %.2f seconds\n', toc);
...
mat_list = who(data);
if any(strcmp(mat_list, 'Ysiz'))
    Ysiz = data.Ysiz;
else
    fprintf('which one is the video mat?\n')
    for i = 1:length(mat_list)
        fprintf('%d. %s\n', i, mat_list{i});
    end
    ii = input('input index\n');
    Ysiz = size(data, mat_list{ii});
end

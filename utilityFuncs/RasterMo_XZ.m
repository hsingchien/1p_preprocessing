function RasterMo_XZ(app, params)
% fields of params: 
% filename, the saving file name
% path, saving path
% f2w, frame number to write to video
% stepsize, step size
sapath = params.path;
...
if params.f2w == 0
    fr2write = app.or_size(3);
else
    fr2write = params.f2w;
end

if ~isnan(params.stepsize) & params.stepsize > 0
...
%     ms = E{e}.Ms;
    ...
    ms = app.ms;
    Z = zscore(ms.RawTraces);
    
    cell_label = app.cell_label;
    
    goodCellId = find(cell_label == 1);
    goodCellNum      = length(goodCellId);
    totalCellNum     = length(cell_label);
    
    ds_height  = app.or_size(1);
...
%     vid = VideoReader([expPath{e}, 'ConcatenatedCrop\msCam\msvideo_smooth.avi']);
    ...
    vid = app.vmat;
    numFrames = app.or_size(3);
    
    % change step
    frameRange = 1:stepsize:fr2write; %numFrames; %%%%% DEFINE FRAME RANGE %%%%%
    
    pxMax = max(ms.SFPs(:));
    
    vidFrame = [];
%     [raw_width, raw_height] = size(read(vid, 1));
    raw_width = size(app.vmat, 1);
    raw_height = size(app.vmat, 2);
...
%     end
    C = VideoWriter([sapath, fname]);
    C.FrameRate = 15; % 5x speed speed = stepsize * c.fr / original fr
    open(C)
...
%     end
    fw = waitbar(0, 'Please wait while generating the raster video');
...
%         I_combi = insertText(I_combi, [ds_width*2+20, 831], ['Video ', num2str(videoID)], 'FontSize',17, 'AnchorPoint', 'RightTop');
        ...
        if mod(fr-1, 1800) == 0
            ...
            imshow(I_combi)
            if mod(fr-1, 9000) == 0
                imwrite(I_combi, [sapath, 'msCam_ROI\ScreenShot', '-', num2str(fr), '.tif'], 'tif')
            end
            if fr == 1
                imwrite(I_combi, [sapath, 'msCam_ROI\ScreenShot_fr1_', '.tif'], 'tif')
            end
            toc; tic;
        end
        writeVideo(C, I_combi); 
        
        waitbar(fr/frameRange(end), fw, 'Please wait while generating the raster video');
    end

...
for i = 1:size(ml,1)
    ...
    firstc = ml(i,1);
    secondc = ml(i,2);
    newcF = [ms.FiltTraces(1:tl,firstc);ms.FiltTraces(tl+1:end,secondc)];
    newcR = [ms.RawTraces(1:tl, firstc); ms.RawTraces(tl+1:end,secondc)];
    newSFP = ms.SFPs(:,:,firstc) + ms.SFPs(:,:,secondc);
    newcS = [ms.S(firstc, 1:tl), ms.S(secondc,tl+1:end)];

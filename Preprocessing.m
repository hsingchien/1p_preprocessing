clear all
dir_f = 'D:\Homecage_free\2020_10_23\XZ2\16_17_49\Miniscope';
vtype = 'm';
if exist(strcat(dir_f, '\', 'video_total.mat'))
    load(strcat(dir_f, '\', 'video_total.mat'));
else
    vmat_total = VideoCombine(dir_f, vtype, true);
end
%% dF/F video
% bg_f = mean(double(vmat_total(:,:,1:200)), 3);
f = figure;
a = axes;
for i = 1:size(vmat_total, 3)
    bg_f = mean(double(vmat_total(:,:,max(i-261, 1):i-61)), 3);
    
    
    current_f = double(vmat_total(:,:,i));
    imagesc(a, (current_f - bg_f)./bg_f, [0,0.5]); 
    pause(0.01) 
end

for i =1:27000
    current_f = double(Mr(:,:,i));
    current_f = 2*(current_f - bg_f)./bg_f;
    current_f(current_f<0) = 0;
    current_f(current_f>1) = 1;
    writeVideo(v, current_f);
end
%% motion correction
...
if (0)    
    ...
    for t = 1:T
        Y(:,:,t) = filter2(hSmall,vmat_total(:,:,t)) - filter2(hLarge, vmat_total(:,:,t));
    end
    %Ypc = Yf - Y;
    bound = size(hLarge,1);
else
    ...
    %bound = 2*ceil(gSiz/2);
    Y = imfilter(vmat_total,psf,'symmetric');
    bound = 0;
end
...
tic; [M1,shifts1,template1] = normcorre_batch(Y(bound/2+1:end-bound/2,bound/2+1:end-bound/2,:),options_r); toc % register filtered data
    % exclude boundaries due to high pass filtering effects
tic; Mr = apply_shifts(vmat_total,shifts1,options_r,bound/2,bound/2); toc % apply shifts to full dataset
    % apply shifts on the whole movie
%% compute metrics 
[cY,mY,vY] = motion_metrics(Y(bound/2+1:end-bound/2,bound/2+1:end-bound/2,:),options_r.max_shift);
[cYf,mYf,vYf] = motion_metrics(vmat_total,options_r.max_shift);
...
tic; Mpr = apply_shifts(vmat_total,shifts2,options_nr,bound/2,bound/2); toc % apply the shifts to the removed percentile
